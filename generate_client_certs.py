...
if __name__ == "__main__":
    ...
    users_file = os.path.join(vpn_config.getConfigDirectory(), 'openvpn_users')

    if not os.path.exists(users_file):
        print(f"{users_file} doesn't exist!")
        sys.exit(1)

    with open(users_file, 'r') as file:
        usernames = file.read().split()
        os.makedirs(vpn_config.getZipDirectory(), exist_ok = True)
